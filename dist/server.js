...
var genOtp = () => crypto.randomInt(1e5, 1e6);
...
var registeUserService = async (paylaod) => {
  ...
  const templatePath = path2.join(process.cwd(), "src/views/verify-email.ejs");
...
var verifyRegistrationEmail = catchAsync_default(
  async (req, res) => {
    ...
    const templatePath = path3.join(process.cwd(), "src/views/welcome.ejs");
...
import z2 from "zod";
var createWorkspaceSchema = z2.object({
  name: z2.string().min(3, { message: "Workspace name must be at least 3 characters" }).max(100, {
...
import z3 from "zod";
var sendInvitationSchema = z3.object({
  //   workspace_id: z.string(),
  member_email: z3.string().email(),
  role: z3.enum(["MEMBER", "ADMIN"], {
...
var sendInvitationService = async (payload) => {
  ...
  const templatePath = path5.join(process.cwd(), "src/views/invitation.ejs");
  const html = await ejs4.renderFile(templatePath, {
    invitationLink,
...
// src/module/user/workspace/invite/invite.route.ts
var invitationRouter = express2.Router();
...
// src/module/user/workspace/member/member.route.ts
var memberRouter = express3.Router();
memberRouter.get(
  "/:id/members",
  verifyUser_default(PlatformRole.USER),
...
var CreateProjectSchema = z4.object({
  name: z4.string().min(1, { message: "Project name cannot be empty" }).max(100, { message: "Name must be smaller than 100 charecters" }),
  description: z4.string().min(1, { message: "Project description cannot be empty" }).max(999, { message: "Description must be smaller than 1000 charecters" })
});
var UpdateProjectSchema = z4.object({
  name: z4.string().min(1, "Project name cannot be empty").optional(),
...
var createSprintSchema = z5.object({
  name: z5.string().min(1, { message: "Name cannot be empty" }),
  startDate: z5.string(),
  endDate: z5.string()
});
var updateSprintSchema = z5.object({
  name: z5.string().min(1, { message: "Name cannot be empty" }).optional(),
...
var createTaskSchema = z6.object({
  title: z6.string({ message: "Task title is required" }).min(1),
  description: z6.string().optional(),
  status: z6.enum(["TODO", "IN_PROGRESS", "REVIEW", "DONE"]).optional().default("TODO")
});
var updateTaskSchema = z6.object({
  title: z6.string().optional(),
  description: z6.string().optional(),
  status: z6.enum(["TODO", "IN_PROGRESS", "REVIEW", "DONE"]).optional()
});
...
// src/module/user/workspace/project/sprint/task/task.route.ts
var taskRouter = express4.Router({ mergeParams: true });
taskRouter.get(
  "/:sprintId/tasks",
  verifyUser_default(PlatformRole.USER),
...
// src/module/user/workspace/project/sprint/sprint.route.ts
var sprintRouter = express5.Router({ mergeParams: true });
sprintRouter.get(
  "/:projectId/sprint",
  verifyUser_default(PlatformRole.USER),
...
var isError = false;
var gemini = new GoogleGenAI({ apiKey: GEMINI_API_KEY });
...
// src/module/user/workspace/project/ai/ai.route.ts
var aiRouter = express6.Router({ mergeParams: true });
aiRouter.post(
  "/ai/chat",
...
// src/module/user/workspace/project/project.route.ts
var projectRouter = express7.Router();
projectRouter.get(
  "/:workspaceId/project",
  verifyUser_default(PlatformRole.USER),
...
var stripe = new Stripe(credentials.stripe_secret_key);
var plans = {
  PROFESSIONAL: {
...
// src/module/user/payment/payment.route.ts
var paymentRouter = express8.Router();
paymentRouter.post(
  "/checkout",
  verifyUser_default(PlatformRole.USER),
...
// src/module/admin/panel/panel.route.ts
var adminPanelRouter = express9.Router();
