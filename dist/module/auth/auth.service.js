...
const registeUserService = async (paylaod) => {
    ...
    const isUserAlredyStoredInRedis = await redisClient.get(`unverified_user:${email}`);
    const isOtpAlreadyStoredInRedis = await redisClient.get(`verify_otp:${email}`);
    ...
    await redisClient.set(`unverified_user:${email}`, JSON.stringify({
        username,
        email,
...
const verifyRegistrationOtpService = async (payload) => {
