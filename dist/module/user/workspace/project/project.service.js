...
const getProjectService = async (payload) => {
...
const createProjectService = async (payload) => {
    ...
    if (isWorkspaceExists.owner.package === "STARTER" &&
        project_count.length >= 2) {
        throw new AppError("Please upgrade your package to create more projcet", 400);
    }
    if (isWorkspaceExists.owner.package === "PROFESSIONAL" &&
...
const updateProjectService = async (payload) => {
