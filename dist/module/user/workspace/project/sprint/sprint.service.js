...
const getSprintService = async (payload) => {
    ...
    if (!isMember) {
        throw new AppError("You are not authorized to view sprints for this project", 403);
    }
...
const createSprintService = async (payload) => {
    ...
    if (isMember?.role !== "OWNER" && isMember?.role !== "ADMIN") {
        throw new AppError("You are not authorized to create sprints for this project", 403);
    }
...
const updateSprintService = async (payload) => {
    ...
    if (isMember?.role !== "OWNER" && isMember?.role !== "ADMIN") {
        throw new AppError("You are not authorized to update sprints for this project", 403);
    }
...
const deletedSprintService = async (payload) => {
    ...
    if (isMember?.role !== "OWNER" && isMember?.role !== "ADMIN") {
        throw new AppError("You are not authorized to delete sprints for this project", 403);
    }
    const existingSprint = await prisma.sprint.findFirst({
        where: {
            id: sprintId,
