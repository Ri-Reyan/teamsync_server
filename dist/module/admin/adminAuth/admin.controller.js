...
import { adminLoginSchema, createAdminSchema, verifyAdminSchema, } from "./admin.schema.js";
const login = catchAsync(async (req, res) => {
    const result = adminLoginSchema.safeParse(req.body);
    if (!result.success)
        throw new AppError(result.error.issues[0].message, 400);
    const admin = await adminAuthService.loginAdmin(result.data.email, result.data.password);
    const tokenPayload = {
        id: admin.id,
    ...
    sendCookie(res, "accessToken", generateToken(credentials.jwt_access_token_secret, tokenPayload, credentials.jwt_access_token_expires));
    sendCookie(res, "refreshToken", generateToken(credentials.jwt_refresh_token_secret, tokenPayload, credentials.jwt_refresh_token_expires));
...
const verifyAdmin = catchAsync(async (req, res) => {
    ...
    if (!result.success)
        throw new AppError(result.error.issues[0].message, 400);
    const admin = await adminAuthService.verifyAdminCreation(result.data.email, result.data.otp);
    return sendResponse(res, {
        success: true,
        statusCode: 201,
