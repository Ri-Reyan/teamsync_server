...
const pendingAdminKey = (email: string) => `unverified_admin:${email}`;
const adminOtpKey = (email: string) => `verify_admin_otp:${email}`;
...
const verifyAdminCreation = async (email: string, otp: string) => {
  ...
  const adminData = JSON.parse(pendingAdmin) as {
    username: string;
    email: string;
    password: string;
  };
